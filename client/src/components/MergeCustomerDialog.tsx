 ...
 *
 * When `customerB` is omitted, a searchable customer picker is shown first
 * so the user can select the merge target from the full customer list.
 */
...
export default function MergeCustomerDialog({ open, onOpenChange, customerA, customerB: initialB, onMerged }: Props) {
  const { state } = useEstimator();
  const [pickedB, setPickedB] = useState<Customer | null>(null);
  const [pickerSearch, setPickerSearch] = useState('');
  const customerB = initialB ?? pickedB;
  ...
  const nameB = customerB
    ? (customerB.displayName || `${customerB.firstName} ${customerB.lastName}`.trim() || 'Customer B')
    : '';

  // Picker list — all customers except customerA, filtered by search
  const pickerList = useMemo(() => {
    const q = pickerSearch.toLowerCase();
    return state.customers
      .filter(c => c.id !== customerA.id && !(c as any).mergedIntoId)
      .filter(c => {
        if (!q) return true;
      ...
      .slice(0, 50);
  }, [state.customers, customerA.id, pickerSearch]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        ...
        {/* ── Step 1: Pick the second customer if not provided ── */}
        {!customerB ? (
          <div className="space-y-3 mt-2">
            <p className="text-sm font-medium">Select the customer to merge with <span className="text-primary">{nameA}</span>:</p>
            ...
            <div className="max-h-64 overflow-y-auto border border-border rounded-lg divide-y divide-border">
              {pickerList.length === 0 ? (
                ...
                return (
                  <button
                    key={c.id}
                    onClick={() => setPickedB(c)}
                    className="w-full text-left px-3 py-2.5 hover:bg-muted transition-colors flex items-center gap-3"
                  >
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium truncate">{name}</div>
                      <div className="text-xs text-muted-foreground truncate">{c.email || c.mobilePhone || [c.city, c.state].filter(Boolean).join(', ') || '—'}</div>
                    </div>
                    {c.company && <Badge variant="secondary" className="text-[10px] shrink-0">{c.company}</Badge>}
                  </button>
                );
              })}
            </div>
          ...
          <>
            ...
            <DialogFooter className="mt-4">
              {!initialB && (
                <Button variant="ghost" size="sm" onClick={() => setPickedB(null)} className="mr-auto text-xs">
                  ← Change selection
                </Button>
              )}
              <Button variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button>
              <Button
                ...
                disabled={mergeMutation.isPending || !sourceId || !targetId}
              >
