...
export default function PresentSection() {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-gray-800/95 overflow-y-auto print:bg-white print:overflow-visible print:static print:inset-auto print:block">
      ...
      <div className="no-print sticky top-0 z-10 bg-gray-900 text-white shadow-lg">
        {/* Row 1: back + title */}
        <div className="flex items-center gap-2 px-3 pt-2.5 pb-1 min-w-0">
          ...
          <div className="flex-1 min-w-0 text-sm font-semibold truncate">
            Present — {jobInfo.client || 'Client'} · {jobInfo.jobNumber}
          </div>
        </div>
        {/* Row 2: action buttons — wraps on very small screens */}
        <div className="flex flex-wrap items-center gap-1.5 px-3 pb-2.5">
          {/* Column visibility toggle */}
          <div className="relative">
            <button
              onClick={() => setShowColPanel(s => !s)}
              className={`flex items-center gap-1 px-2 py-1.5 border rounded text-xs font-semibold transition-colors ${showColPanel ? 'bg-white/20 border-white/40' : 'border-white/20 hover:bg-white/10'}`}
          ...
          {state.signature ? (
            <>
              <div className="flex items-center gap-1 text-emerald-400 text-xs font-semibold">
                ...
                <span className="hidden sm:inline">Signed by {state.signedBy}</span>
                <span className="sm:hidden">Signed</span>
              </div>
              <button
                onClick={() => setShowApprovedModal(true)}
                className="flex items-center gap-1 px-2 py-1.5 bg-amber-500 hover:bg-amber-600 rounded text-xs font-semibold text-white transition-colors"
              >
              ...
              <button onClick={() => clearSignature()} className="px-2 py-1.5 text-xs border border-white/20 hover:bg-white/10 rounded transition-colors">
                <span className="hidden sm:inline">Clear Sig</span>
            ...
            <button
              ...
              className="flex items-center gap-1 px-2 py-1.5 bg-emerald-600 hover:bg-emerald-700 rounded text-xs font-semibold transition-colors"
            >
          ...
          <button
            ...
            className="flex items-center gap-1 px-2 py-1.5 bg-green-600 hover:bg-green-700 rounded text-xs font-semibold transition-colors"
          >
            <Send className="w-3.5 h-3.5" />
            <span className="hidden sm:inline">Send to Customer</span>
            <span className="sm:hidden">Send</span>
          </button>
          <button onClick={handlePrint} className="flex items-center gap-1 px-2 py-1.5 border border-white/20 hover:bg-white/10 rounded text-xs font-semibold transition-colors">
