 ...
 *
 * The DB calls here live in this module rather than processor.ts so they
 * can be unit-tested against a mocked Drizzle instance without mocking the
 * whole pipeline.
 *
 * TODO(wire-up): the `db` passed in must be the project's Drizzle instance
 * (server/db.ts). Currently imported lazily via the caller to avoid a hard
 * dependency on a module that is missing from origin/main.
 */
...
export type DbLike = NodePgDatabase<any>;
