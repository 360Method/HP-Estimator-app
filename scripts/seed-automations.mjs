...
const RULES = [
  ...
  },
  {
    ...
    actionPayload: JSON.stringify({
      ...
      bodyTemplate:
        'Hi {{customerFirstName}},\n\nThank you for reaching out to Handy Pioneers! We received your service request for {{description}} and a team member will be in touch within 1 business day.\n\nIf you have any questions in the meantime, feel free to reply to this email.\n\nBest,\nThe Handy Pioneers Team',
    }),
    delayMinutes: 0,
    sortOrder: 11,
  },
...
for (const rule of RULES) {
  ...
  await conn.execute(
    `INSERT INTO automationRules
       (name, \`trigger\`, actionType, actionPayload, delayMinutes, enabled, sortOrder, conditions)
     VALUES (?, ?, ?, ?, ?, 0, ?, '[]')`,
    [rule.name, rule.trigger, rule.actionType, rule.actionPayload, rule.delayMinutes, rule.sortOrder]
  );
